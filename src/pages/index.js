...
export default function Home() {
  ...
  const { fetchFeaturedProductsRequest, fetchBestSellerProductsRequest, fetchDaleProductsRequest, fetchSaleProductsRequest } = shopActions;
  const shopState = useSelector((state) => state.shopReducer);
  const { featuredProducts, bestSellerProducts, saleProducts, daleProducts } = shopState;
  useEffect(() => {
  ...
  return (
    <LayoutThree title="Homepage 3" headerClass="-pink -no-space" footerClass="-pink">
        ...
        <Row gutter={[30, 30]}>
          <Col xs={24} sm={24} lg={6}>
            <ProductSlide
              ...
              column
            />
            <Benefits style={{ marginBottom: 30 / 16 + "em" }} className="-pink -bordered" column />
            <ProductGrid style={{ marginBottom: 30 / 16 + "em" }} headerTitle="Sale products" headerClass="-small -underline" productClassName="-bordered" productCol={{ xs: 24, sm: 24 }} gutter={0} className="-pink" productType="tiny" data={saleProducts} />
            <TestimonialOne />
          </Col>
